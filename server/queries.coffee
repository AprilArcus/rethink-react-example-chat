cfg = require('./config')
{ RQ } = require('rethinkdb-websocket-server')
...
queryWhitelist = [
    ...
    RQ.FILTER(
      ...
      RQ.FUNC(
        ...
        RQ.GE(
          ...
          RQ.ISO8601((x) -> typeof x is 'string')
        )
      )
    )
  ...
  .validate((refs, session) ->
    refs.userId is session.userId and
    typeof refs.body is 'string' and
    refs.body.trim().length > 0
  )

]

module.exports = queryWhitelist: queryWhitelist
